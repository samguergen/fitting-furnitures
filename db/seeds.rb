sam = {:email => "sam@gmail.com", :password => "12", :password_confirmation => "12", :location => "Paris, France", :name => "Samantha Guergenenov", :age => 24, :gender => "female", :description => "girl next door" :avatar => "http://toons.mit.edu/images/c/c7/Samantha.jpg"}
malcolm = {:email => "malcolm@gmail.com", :password => "34", :password_confirmation => "34", :location => "New York, NY", :name => "Malcolm Bouzi", :age => 24, , :gender => "male", :description => "cool guy", :avatar => "https://pbs.twimg.com/profile_images/549954802185039872/6x9QAsgg.jpeg"}
kevin = {:email => "kevin@gmail.com", :password => "56", :password_confirmation => "56", :location => "JockVille, MA", :name => "Kevin Alwell", :age => 13, , :gender => "male", :description => "prettu shy", :avatar => "data:image/jpeg;base64,"}
hoa = {:email => "hoa@gmail.com", :password => "78", :password_confirmation => "78", :location => "Mars", :name => "Hoa Nguyen", :age => 26, , :gender => "male", :description => "please bother me! I am bored.", :avatar => "http://www.uni-tuebingen.de/Klinische_Genetik/img/_personen/huu_phuc_nguyen-hoa.jpg"}

user_1 = User.create!(sam)
user_2 = User.create!(malcolm)
user_3 = User.create!(kevin)
user_4 = User.create!(hoa)

furni1 = {:name => "drawer", :content => "Two-tone finish in white wash replicated worn through paint with authentic touch and a replicated wood grained block pattern with authentic touch. Large scaled shapely bedroom collection features bun feet.", :condition => "like new", :dimensions => "100,150", :image_url => "https://encrypted-tbn1.gstatic.com/shopping?q=tbn:ANd9GcSRD12s37wPslC9MUn7JJFvD9YIMWwL4JvSMR-vj2vx8i46pqaQp9Q5ENqH2imF5axLRYDjDQc&usqp=CAE", :furnipoints => 20}
furni2 = {:name => "bed", :content => "The Memoir 12 mattress will support body weight evenly, relieve pressure focused on the shoulders, hips, and feet, and eliminate motion disturbance. ", :condition => "used", :dimensions => "50,200", :image_url => "https://encrypted-tbn0.gstatic.com/shopping?q=tbn:ANd9GcSW2-e3SA1Cvnq4J6zuNgs7_JN8IAvMBS46y0K2T7iBQBT-iFs&usqp=CAE", :furnipoints => 30}
furni3 = {:name => "desk", :content => "This beautiful desk is constructed of sturdy wood and comes in a diamond ash finish that complements any style decor. It has a spacious lower file drawer that accommodates letter-size hanging files so all your important documents can be neatly stored away.", :condition => "very used", :dimensions => "75,30", :image_url => "https://encrypted-tbn2.gstatic.com/shopping?q=tbn:ANd9GcTRxRAPQxJ07iYJ05kM8xAmtxFu80MdlElxtvcwJa3GM-xR9jyQc2iOsNJSCg&usqp=CAE", :furnipoints => 15}
furni4 = {:name => "chair", :content => "Multipurpose Stack Chair Contemporary Design 1000 lb. Weight Capacity Stacks up to 30 Chairs High Black Fabric Upholstery Lightweight Design ", :condition => "correct", :dimensions => "50,20", :image_url => "https://encrypted-tbn0.gstatic.com/shopping?q=tbn:ANd9GcTKBsudctxRovjCcJOSA9YU1jAJKi_qg19EGVrSQkDPKyXK-1JP4mEymHRIMgj5J-eEE8SIonk&usqp=CAE", :furnipoints => 5}

user_1.update_attributes(:furniture_id => furni1.id)
user_2.update_attributes(:furniture_id => furni2.id)
user_3.update_attributes(:furniture_id => furni3.id)
user_4.update_attributes(:furniture_id => furni4.id)

post1 = {:title => "World domination", :content => "What are your best plans for world domination?" , :user_id => user_1.id}
post2 = {:title => "Ruby vs. Java", :content => "What is the difference between Ruby and Java?" , :user_id => user_2.id }
post3 = {:title => "Put", :content => "What does put do in Ruby?" , :user_id => user_3.id}
post4 = {:title => "Functional language", :content => "What is an example of a functional language?" , :user_id => user_4.id}

post_1 = Post.create!(post1)
post_2 = Post.create!(post2)
post_3 = Post.create!(post3)
post_4 = Post.create!(post4)

answer1 = {:content => "You should perfect your evil laugh", :post_id => post_1.id , :user_id => user_4.id }
answer2 = {:content => "Ruby is a dynamic language, Java is a static one.", :post_id => post_2.id , :user_id => user_1.id }
answer3 = {:content => "Put is like print but with an extra line", :post_id => post_3.id , :user_id => user_2.id }
answer4 = {:content => "Javascript", :post_id => post_4.id , :user_id => user_3.id }


answer_1 = Comment.create!(answer1)
answer_2 = Comment.create!(answer2)
answer_3 = Comment.create!(answer3)
answer_4 = Comment.create!(answer4)

post_1.update_attributes(:comment_id => answer_1.id)
post_2.update_attributes(:comment_id => answer_2.id)
post_3.update_attributes(:comment_id => answer_3.id)
post_4.update_attributes(:comment_id => answer_4.id)
